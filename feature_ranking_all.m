...
for iter = 1 : 10
    ...
    selectedFeatures = feast('mrmr',10,Data(:,[4:11, 39:44]),label);
    
    Ranks(iter).featureNames = ft(:,[4:11, 39:44]).Properties.VariableNames(selectedFeatures);
    Ranks(iter).featurelist = selectedFeatures;
    
end
toc
matfile = sprintf('%s','Ten_unbalanced_feature_ranks_mrmr');
save(matfile, 'Ranks')