...
% Find DMI samples
cvi_idx = find (labels == 1); % They are 42 in total
...
cvi_data_length = size(cvi_data,1);
noncvi_data_length = size(noncvi_data,1);

rand_idx = randperm (noncvi_data_length , cvi_data_length);

rand_noncvi_data = noncvi_data (rand_idx,:);
% rand_nondmi_labels = nondmi_labels ( rand_idx);

% Split into 7 sets each of size 6
N = 7;
k = 6;

% Include the labels too

cvi_pieces = reshape ( cvi_data', [numFeatures,N,k] );
noncvi_pieces = reshape ( rand_noncvi_data', [numFeatures,N,k] );
...
for i = 1 : k
    Data = vertcat (Data, cvi_pieces(:,:,i)' , noncvi_pieces(:,:,i)');
    label = vertcat (label, ones(size(cvi_pieces(:,:,i),2),1) , zeros(size(noncvi_pieces(:,:,i),2),1));
end
    ...
    for jj = 1 : length (feature_comb)
        ...
        for kk = 1 : k
            ...
            classes = svmclassify(svmStruct,val_data(find(test),:),...
                'showplot',false);
%             error(jj,kk) = ( classes == val_data((train)))./sum(train)
        ...
        results = vertcat(results, [ii, jj, models{high_idx(1)}.CorrectRate]);
        ...
        % For two features
        % plotConfMat(cfMat(:,:,7),{'1','0'}) % 100 % accuracy
        
        % For one feature
        % plotConfMat(cfMat(:,:,1),{'1','0'})92.86 % percent accuracy
