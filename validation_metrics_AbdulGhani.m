...
% This code stores validation results on the hold out data. Ten iterations
...
for iter = 1 : size(filenames,1)
    ...
    for NbF = 1 : numFeatures
        ...
        Validation_results(iter).max_acc(NbF) = accmax;
        Validation_results(iter).max_acc_idx_r(NbF) = accmax_r;
        Validation_results(iter).max_acc_idx_c(NbF) = accmax_c;
        Validation_results(iter).sp_max_acc(NbF) = SP_max_ACC;
        Validation_results(iter).sn_max_acc(NbF) = SN_max_ACC;
        
        Validation_results(iter).max_sp(NbF) = spmax;
        Validation_results(iter).max_sp_idx_r(NbF) = spmax_r;
        Validation_results(iter).max_sp_idx_c(NbF) = spmax_c;
        Validation_results(iter).acc_max_sp(NbF) = ACC_max_SP;
        Validation_results(iter).sn_max_sp(NbF) = SN_max_SP;
        
        Validation_results(iter).max_sn(NbF) = snmax;
        Validation_results(iter).max_sn_idx_r(NbF) = snmax_r;
        Validation_results(iter).max_sn_idx_c(NbF) = snmax_c;
        Validation_results(iter).acc_max_sn(NbF) = ACC_max_SN;
        Validation_results(iter).sp_max_sn(NbF) = SP_max_SN;
        
        Validation_results(iter).CM_acc{NbF} = CM_acc;
        Validation_results(iter).CM_sn{NbF} = CM_sn;
        Validation_results(iter).CM_sp{NbF} = CM_sp;
        Validation_results(iter).features{NbF} = features;
        clearvars -except Validation_results iter M2 filenames
...
save Validation_results_AbdulGhani_20 Validation_results
