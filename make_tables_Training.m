...
for i = 1 : numFeatures
    ...
    for j = 1 : numIterations
        ...
        tr_CM_acc{i,j} = Training_results(j).CM_acc{i};
...
for ii= 1:numFeatures
    tr_idx_max_acc{ii} = find(tr_ACC(ii,:)== max_tr_ACC(ii));
   % tr_CM_MAX_ACC{ii} = tr_CM_acc(tr_idx_max_acc);
end 
...
grid on; box on
matlab2tikz('filename',sprintf('AbdulGhani_Acc.tex'));
...
grid on; box on
matlab2tikz('filename',sprintf('AbdulGhani_SEN.tex'));
...
hold on
errorbar(1:10,mean_tr_SP,std_tr_SP,...
    'LineWidth', 1.4)
errorbar(1:10,mean_val_SP,std_val_SP,...
    'LineWidth', 1.4)
...
legend('Training', 'Validation','location','southeast');
grid on; box on
matlab2tikz('filename',sprintf('AbdulGhani_SP.tex'));