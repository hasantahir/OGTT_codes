...
for iter = 1 : 25
    ...
    matfile = sprintf('%s_%d','AbdulGhani_unbalanced_models_new',iter); 
    load(matfile, 'M_all')
    ...
    for NbF = 1 : numFeatures
        ...
        Training_plus_Validation_results(iter).trCM_acc{NbF} = tr_CM_acc;
        Training_plus_Validation_results(iter).trCM_sn{NbF} = tr_CM_sn;
        Training_plus_Validation_results(iter).trCM_sp{NbF} = tr_CM_sp;
        Training_plus_Validation_results(iter).valCM_acc{NbF} = val_CM_acc;
        Training_plus_Validation_results(iter).valCM_sn{NbF} = val_CM_sn;
        Training_plus_Validation_results(iter).valCM_sp{NbF} = val_CM_sp;
...
save Training_plus_Validation_ABD_10iters Training_plus_Validation_results
