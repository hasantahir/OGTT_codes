...
for NbF = 1 : 1
    len = size(M3(NbF).all_cv_models,2);
    
    all_cv_models = M3(NbF).all_cv_models;
    ...
    sn_accmax = SN(accmax_r, accmax_c)
    sp_accmax = SP(accmax_r, accmax_c)
    % [sp,I] = max(SP);
    ...
    % Corresponding sensitivities and specificities
    acc_spmax = ACC(spmax_r, spmax_c)
    sn_spmax = SN(spmax_r, spmax_c)
