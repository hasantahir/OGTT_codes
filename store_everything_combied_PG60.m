...
for iter = 1 : 100
    ...
    DMI_train = FT{idx(1:150), :};
    DMI_test = FT{idx(151:end), :}; % this is used for validation
...
%     non_pieces = reshape ( rand_non_data', [numFeatures,N_nodmi,k] );
    ...
    Data = [DMI_train; rand_non_data];
    label = [ones(size(DMI_train)); zeros(size(rand_non_data))];
%     pieces = reshape ( DMI_train, [N_dmi, numFeatures,k] );
%     non_pieces = reshape ( rand_non_data', [numFeatures,N_nodmi,k] );
%     
%     Data = [];
%     label = [];
...
%     end
    ...
    val_dmi = FT{idx(151:end), :}; %validation part 1
    ...
    HO_labels = [ones(21,1);zeros(size(Healthydata_test,1),1)];
    ...
    for NbF = 1 : 1 % : 3 %Features Selected (60 in total)
        ...
        features = 1;%combinator(numFeatures,NbF,'c'); % combinator
...
%                     'options',options);
    ...
    matfile = sprintf('%s_%d','PG60_new',iter);
