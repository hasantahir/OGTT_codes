...
load Training_results__unbalanced_AbdulGhani_100iters Training_results
load Validation_results_unbalanced_AbdulGhani_100iters Validation_results
...
for i = 1 : numFeatures
    ...
    for j = 1 : numIterations
        ...
        tr_ACC(i,j) = Training_results(j).max_acc(i);
        tr_SP(i,j) = Training_results(j).sp_max_acc(i);
        tr_SEN(i,j) = Training_results(j).sn_max_acc(i);
        tr_Features{i,j} = char((Training_results(j).features{i}));
        
        val_ACC(i,j) = Validation_results(j).max_acc(i);
        val_SP(i,j) = Validation_results(j).sp_max_acc(i);        
...
matlab2tikz('filename',sprintf('AbdulGhani_val_ACC_unbalanced.tex'));
hgexport(gcf, 'AbdulGhani_val_ACC_unbalanced.jpg', hgexport('factorystyle'), 'Format', 'jpeg');
savefig('AbdulGhani_val_ACC_unbalanced.fig')
...
matlab2tikz('filename',sprintf('AbdulGhani_tra_ACC_unbalanced.tex'));
hgexport(gcf, 'AbdulGhani_tra_ACC_unbalanced.jpg', hgexport('factorystyle'), 'Format', 'jpeg');
savefig('AbdulGhani_tra_ACC_unbalanced.fig')
